...
import {
  ...
  Search, Filter, Edit2, Check, BarChart2, ClipboardList, Bell, Share2, MessageCircle
} from 'lucide-react';
import { supabase } from './supabaseClient';

// ─── ENVIRONMENT VARIABLES ───
// CRITICAL FIX #8: No hardcoded fallback — the password must come from an
// environment variable (e.g. VITE_ADMIN_PASSWORD set in Vercel project settings).
// A hardcoded fallback here would still ship inside the JS bundle and be
// readable in DevTools → Sources, defeating the purpose of using env vars.
const ADMIN_PASSWORD = import.meta.env.VITE_ADMIN_PASSWORD;
const WHATSAPP_NUMBER = '03442035118';
...
// ─── FEATURE 33: WHATSAPP SHARE & ORDER ───
function shareProductOnWhatsApp(product) {
  const baseUrl = window.location.origin;
  const productId = product.id;
  // Create shareable product URL (will need routing setup)
...
function sendOrderViaWhatsApp(orderData) {
  ...
  const msg = `📦 *ORDER PLACED* on Lyfelytic\n\n*Customer Details:*\nName: ${orderData.customer_name}\nPhone: ${orderData.customer_phone}\nEmail: ${orderData.customer_email}\nCity: ${orderData.customer_city}\nAddress: ${orderData.customer_address}\n\n*Items:*\n${items}\n\n*Total:* Rs.${orderData.total_price}\n\n✅ Your order has been received. We'll deliver soon!`;
  window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(msg)}`, '_blank');
}

// ─── CART PERSISTENCE ───
function loadCart() {
  try { return JSON.parse(localStorage.getItem('lyf_cart') || '[]'); } catch { return []; }
}
function saveCart(c) { localStorage.setItem('lyf_cart', JSON.stringify(c)); }
...
function Toast({ toasts }) {
  return (
    <div style={{ position: 'fixed', bottom: 90, left: '50%', transform: 'translateX(-50%)', zIndex: 9998, display: 'flex', flexDirection: 'column', gap: 8, alignItems: 'center', pointerEvents: 'none' }}>
      {toasts.map(t => (
        <div key={t.id} style={{
          background: t.type === 'error' ? '#1a0a10' : t.type === 'success' ? '#071a10' : '#07101a',
          border: `1px solid ${t.type === 'error' ? 'rgba(255,94,117,0.4)' : t.type === 'success' ? 'rgba(34,197,94,0.4)' : 'rgba(36,201,255,0.4)'}`,
          color: t.type === 'error' ? '#ff5e75' : t.type === 'success' ? '#22c55e' : '#24c9ff',
...
function SkeletonCard() {
  return (
    <div style={{ background: '#090e14', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 6, overflow: 'hidden' }}>
      <div style={{ width: '100%', height: 240, background: '#0d1219', animation: 'shimmer 1.4s ease-in-out infinite' }} />
      <div style={{ padding: 14 }}>
        {[40, 80, 60, 30].map((w, i) => (
          <div key={i} style={{ height: i === 1 ? 14 : 11, width: w + '%', borderRadius: 4, background: '#131c26', marginBottom: 8 }} />
        ))}
        <div style={{ height: 34, borderRadius: 8, background: '#131c26', marginTop: 4 }} />
      </div>
    </div>
  );
}
...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  /* PERF FIX: this cursor-glow effect is meaningless on touch devices (no
     mouse) and its fixed + blend-mode layer adds a real compositing cost
     during scroll on mobile GPUs — safe to turn off where there's no mouse. */
  ...
  /* PERF FIX: backdrop-filter blur on a sticky element is a heavy repaint
     cost on mobile GPUs — a lighter blur + slightly more opaque background
     keeps the same look while being much cheaper to render during scroll. */
  @media (max-width: 850px) {
    .lyfelytic-header { backdrop-filter: blur(8px); background: rgba(4,6,9,.94); }
  }
  ...
  /* IMPORTANT #9: Mobile search — quick access from the sticky header */
  .mobile-search-btn { background: none; border: none; color: #fff; cursor: pointer; display: flex; align-items: center; padding: 4px; margin-right: 6px; }
  @media (min-width: 850px) { .mobile-search-btn { display: none; } }
  .mobile-search-bar { display: none; align-items: center; gap: 10px; background: #090d12; border: 1px solid rgba(255,255,255,.08); border-radius: 999px; padding: 10px 16px; margin: 10px 20px 0; }
  .mobile-search-bar.open { display: flex; }
  .mobile-search-bar input { flex: 1; background: none; border: none; outline: none; color: #eaf4ff; font-size: 14px; min-width: 0; }
  .mobile-search-bar input::placeholder { color: #5d6b7f; }
  @media (min-width: 850px) { .mobile-search-bar { display: none !important; } }
  ...
  /* NICE TO HAVE #12: category product count badge */
  .category-count-badge { grid-column: 1/-1; color: #8794a6; font-size: 11px; font-weight: 600; margin-top: 4px; }
...
function ScrollProductStage() {
  ...
  useEffect(() => {
    ...
    // PERF FIX: lower-cost rendering on phones/touch devices — soft shadows,
    // antialiasing, and a high pixel ratio are the biggest GPU cost here and
    // were the main cause of site-wide scroll lag on mobile.
    ...
    let raf;
    // PERF FIX: this loop was rendering the 3D scene at 60fps for the entire
    // time someone browsed the shop page, even after scrolling far past it.
    // Pausing it via IntersectionObserver when off-screen was the single
    // biggest fix for the mobile scroll lag.
    function animate() {
      raf = requestAnimationFrame(animate);
      if (!reduceMotion) rootRig.rotation.y += 0.0004;
      renderer.render(scene, camera);
    }
    function startLoop() { if (!raf) animate(); }
    function stopLoop() { if (raf) { cancelAnimationFrame(raf); raf = undefined; } }
    startLoop();

    const visibilityObserver = new IntersectionObserver((entries) => {
      entries.forEach(entry => { if (entry.isIntersecting) startLoop(); else stopLoop(); });
    }, { threshold: 0 });
...
export default function LyfelyticEcommerce() {
  ...
  // IMPORTANT #6: Product page URL routing — pushes ?product=<id> onto the
  // URL so it can be shared/bookmarked and reloaded directly to that product.
  ...
  // IMPORTANT #6: Closes the product page and removes ?product= from the URL.
  const closeProduct = () => {
  ...
  // IMPORTANT #6: On first load (or when someone opens a shared product link),
  // read ?product=<id> from the URL and open that product once products have
  // loaded. Also listens for browser back/forward so the product page and
  // shop stay in sync with the URL.
  useEffect(() => {
    const syncProductFromUrl = () => {
      ...
      if (productId) {
        ...
        if (products.length === 0) return; // still loading — wait for fetch instead of flashing the shop page
      }
      setSelectedProduct(null);
    };
    syncProductFromUrl();
  ...
  const [imageUploading, setImageUploading] = useState(false); // NICE TO HAVE #13
  ...
  // IMPORTANT #9: Mobile search toggle — lets people search from the sticky
  // header instead of having to scroll past the hero and 3D section first.
  ...
  // ─── NICE TO HAVE #10: Toast notifications ───
  ...
  // NICE TO HAVE #14: Favicon. Injected at runtime since this is a single
  // component file — for a full project, the more standard approach is to
  // drop a real logo PNG into /public and link it from index.html instead.
  useEffect(() => {
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64"><rect width="64" height="64" rx="14" fill="#050608"/><text x="50%" y="58%" font-family="Space Grotesk, Arial, sans-serif" font-size="34" font-weight="700" fill="#24c9ff" text-anchor="middle">L</text></svg>`;
  ...
  useEffect(() => {
    // PERF FIX: skip this on touch devices — there's no mouse to track, and
    // it avoids the extra style writes/paints on every pointer event.
  ...
  const addToCart = (product) => {
    // NICE TO HAVE #10: Show "Added to cart" toast
    setCart(c => [...c, product]);
  ...
  const handlePlaceOrder = async () => {
    const { name, phone, email, address, city } = checkoutForm;
    // CRITICAL FIX #4: Validate email field
    ...
    // CRITICAL FIX #3: Include quantity in items and track stock decrease
    const items = cart.map(i => ({ id: i.id, name: i.name, price: i.price, quantity: i.quantity || 1 }));
    ...
    // CRITICAL FIX #3: Decrease stock for each product
    ...
    // CRITICAL FIX #1: Show order confirmation modal instead of just alert
    const orderData = { 
      id: orderId,
      customer_name: name, 
  ...
  // NICE TO HAVE #13: Upload to Supabase Storage instead of saving base64
  // strings straight into the products table — keeps the database small and
  // the site fast. Requires a public bucket named "product-images" to exist
  // in the Supabase project (Storage → New bucket → make it public).
  const handleImagesUpload = async (e, isEdit = false) => {
    ...
    for (const file of files) {
      ...
      if (publicUrl) {
        if (isEdit) setEditingProduct(prev => ({ ...prev, images: [...(prev.images || []), publicUrl] }));
        else setNewProduct(prev => ({ ...prev, images: [...prev.images, publicUrl] }));
      }
    }
  ...
  const handleAdminClick = () => {
    // CRITICAL FIX #8: fail closed if the env var isn't configured, instead of
    // silently comparing against undefined (which would never match anyway,
    // but this gives a clear signal to whoever is deploying the site).
    if (!ADMIN_PASSWORD) {
      alert('Admin login is not configured. Set VITE_ADMIN_PASSWORD in your environment variables.');
      return;
    }
    const pw = prompt('Admin password:');
    if (pw === ADMIN_PASSWORD) { setIsAdmin(true); setMobileMenuOpen(false); }
        ...
        <header className="lyfelytic-header">
          <div className="header-content">
            ...
            <div className="nav-buttons">
              <button onClick={closeProduct} className="nav-btn nav-btn-shop"><Home size={15} /> Shop</button>
              {/* IMPORTANT #7: Add empty cart feedback */}
              <button className="nav-btn nav-btn-cart" onClick={() => {
                if (cart.length > 0) { setShowCheckout(true); }
                else { addToast('Your cart is empty. Add items to get started!', 'success'); }
              }}>
          ...
          <div className={`mobile-menu ${mobileMenuOpen ? 'open' : ''}`}>
            <button onClick={() => { closeProduct(); setMobileMenuOpen(false); }} className="mobile-menu-btn-item"><Home size={16} /> Shop</button>
            <button onClick={() => { 
              if (cart.length > 0) { setShowCheckout(true); setMobileMenuOpen(false); }
              else { addToast('Your cart is empty. Add items to get started!', 'success'); }
            }} className="mobile-menu-btn-item"><ShoppingCart size={16} /> Cart ({cart.length})</button>
          </div>
        </header>
        <ProductPage product={selectedProduct} onBack={closeProduct} onAddToCart={addToCart} allProducts={products} />
        {showCheckout && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => e.target === e.currentTarget && setShowCheckout(false)}>
            <div style={{ background: '#0a0f16', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 440, maxHeight: '90vh', overflowY: 'auto' }}>
              ...
              <div style={{ background: '#070b10', padding: 14, borderRadius: 8, marginBottom: 16, border: '1px solid rgba(255,255,255,0.06)' }}>
                {cart.length === 0 && <div style={{ textAlign: 'center', color: '#586678', fontSize: 13, padding: '8px 0' }}>Your cart is empty.</div>}
                {cart.map((item, i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, color: '#8794a6', padding: '6px 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                    <span style={{ flex: 1 }}>{item.name}</span>
                    <span style={{ marginRight: 10, color: '#24c9ff', fontWeight: 700 }}>Rs. {item.price}</span>
                    <button onClick={() => removeFromCart(i)} style={{ background: 'rgba(255,94,117,0.12)', border: 'none', color: '#ff5e75', borderRadius: 6, width: 24, height: 24, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      <Trash2 size={12} />
                    </button>
                  </div>
                ))}
              ...
              <input className="form-input" style={{ marginBottom: 10 }} placeholder="Email *" type="email" value={checkoutForm.email} onChange={e => setCheckoutForm({ ...checkoutForm, email: e.target.value })} />
  ...
  return (
    <div className="lyfelytic-container">
      ...
      {/* NICE TO HAVE #11: WhatsApp Floating Button */}
      <button onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}`, '_blank')} style={{
        position: 'fixed', bottom: 24, right: 24, width: 56, height: 56,
        background: '#25d366', border: 'none', borderRadius: '50%',
        cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center',
        zIndex: 400, boxShadow: '0 4px 12px rgba(37,211,102,0.4)',
        transition: 'transform 0.3s ease',
        fontSize: 24
      }} onMouseEnter={e => e.target.style.transform = 'scale(1.1)'} onMouseLeave={e => e.target.style.transform = 'scale(1)'} title="Chat on WhatsApp">
      ...
      {showCheckout && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => e.target === e.currentTarget && setShowCheckout(false)}>
          <div style={{ background: '#0a0f16', border: '1px solid rgba(255,255,255,0.12)', borderRadius: 14, padding: 24, width: '100%', maxWidth: 440, maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowCheckout(false)} style={{ flex: 1, padding: 10, background: 'transparent', border: '1px solid rgba(255,255,255,0.1)', color: '#8794a6', borderRadius: 8, cursor: 'pointer' }}>Cancel</button>
              <button onClick={handlePlaceOrder} disabled={cart.length === 0} style={{ flex: 1.5, padding: 10, background: cart.length === 0 ? 'rgba(255,255,255,0.08)' : '#24c9ff', border: 0, color: cart.length === 0 ? '#586678' : '#050608', borderRadius: 8, fontWeight: 700, cursor: cart.length === 0 ? 'not-allowed' : 'pointer' }}>Confirm Order</button>
            </div>
          </div>
        </div>
      )}

      {/* CRITICAL FIX #1: Order Confirmation Modal */}
      {showOrderConfirmation && lastOrder && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', zIndex: 500, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={() => setShowOrderConfirmation(false)}>
          <div style={{ background: '#0a0f16', border: '1px solid rgba(36,201,255,0.3)', borderRadius: 14, padding: 32, width: '100%', maxWidth: 500, textAlign: 'center' }}>
            <div style={{ fontSize: 48, marginBottom: 16 }}>✅</div>
            <div style={{ fontSize: 24, fontWeight: 700, color: '#24c9ff', marginBottom: 8 }}>Order Confirmed!</div>
            <div style={{ fontSize: 13, color: '#8794a6', marginBottom: 24, lineHeight: 1.6 }}>Thank you for your order, {lastOrder.customer_name}. Your order has been received and sent to our team via WhatsApp.</div>
            
            <div style={{ background: '#070b10', padding: 16, borderRadius: 12, marginBottom: 24, textAlign: 'left', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ fontSize: 11, color: '#586678', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase' }}>Order Details</div>
              <div style={{ fontSize: 13, color: '#fff', marginBottom: 12, paddingBottom: 12, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><span>Order ID:</span><span style={{ color: '#24c9ff' }}>{lastOrder.id}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}><span>Items:</span><span>{lastOrder.items.length}</span></div>
                <div style={{ display: 'flex', justifyContent: 'space-between' }}><span>Total:</span><span style={{ color: '#22c55e', fontWeight: 700 }}>Rs. {lastOrder.total_price}</span></div>
              </div>
              <div style={{ fontSize: 11, color: '#586678', marginBottom: 8, fontWeight: 600, textTransform: 'uppercase' }}>Delivery Details</div>
              <div style={{ fontSize: 12, color: '#d8e0ea', lineHeight: 1.6 }}>
                <div>{lastOrder.customer_name}</div>
                <div>{lastOrder.customer_phone}</div>
                <div>{lastOrder.customer_city}</div>
                <div style={{ color: '#8794a6', fontSize: 11, marginTop: 4 }}>{lastOrder.customer_address}</div>
              </div>
            </div>

            <div style={{ background: '#070b10', padding: 14, borderRadius: 8, marginBottom: 24, fontSize: 12, color: '#8794a6', lineHeight: 1.6 }}>
              📧 A confirmation has been sent to <span style={{ color: '#24c9ff', fontWeight: 600 }}>{lastOrder.customer_email}</span><br/>
              💬 Our team will contact you on <span style={{ color: '#24c9ff', fontWeight: 600 }}>{lastOrder.customer_phone}</span> to confirm delivery details
            </div>

            <div style={{ display: 'flex', gap: 10 }}>
              <button onClick={() => setShowOrderConfirmation(false)} style={{ flex: 1, padding: 12, background: '#24c9ff', border: 0, color: '#050608', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>Continue Shopping</button>
              <button onClick={() => { setShowOrderConfirmation(false); window.open(`https://wa.me/${WHATSAPP_NUMBER}`, '_blank'); }} style={{ flex: 1, padding: 12, background: 'transparent', border: '1px solid rgba(36,201,255,0.4)', color: '#24c9ff', borderRadius: 8, fontWeight: 700, cursor: 'pointer', fontSize: 13 }}>Message on WhatsApp</button>
            </div>
          </div>
        </div>
      )}
      ...
      <header className="lyfelytic-header">
        <div className="header-content">
          ...
          {!isAdmin && view === 'shop' && (
            <button className="mobile-search-btn" onClick={() => setMobileSearchOpen(o => !o)} aria-label="Search">
        ...
        {!isAdmin && view === 'shop' && (
          <div className={`mobile-search-bar ${mobileSearchOpen ? 'open' : ''}`}>
            <Search size={16} color="var(--slate-lt)" />
            <input
              autoFocus={mobileSearchOpen}
              placeholder="Search products..."
              value={searchQuery}
              onChange={e => setSearchQuery(e.target.value)}
              onKeyDown={e => { if (e.key === 'Enter' && productsSectionRef.current) productsSectionRef.current.scrollIntoView({ behavior: 'smooth', block: 'start' }); }}
            />
        ...
        <section className="category-showcase">
          ...
          <div className="category-showcase-grid">
            {categories.slice(0, 4).map((cat, index) => {
              ...
              // NICE TO HAVE #12: product count badge so shoppers know what's inside before clicking
              const categoryCount = products.filter(p => p.category?.toLowerCase() === cat.toLowerCase()).length;
              return (
                <button key={cat} className="category-showcase-card" onClick={() => selectCategoryAndScroll(cat)}>
                  ...
                  <span className="category-card-content">
                    ...
                    <strong>{cat}</strong>
                    <em>Explore <ChevronRight size={14} /></em>
                    <span className="category-count-badge">{categoryCount} {categoryCount === 1 ? 'product' : 'products'}</span>
                  </span>
                </button>
              );
            })}
        ...
        {!isAdmin && view === 'shop' && (
          <div>
            ...
            <div className="products-container">
              <div>
                {/* IMPORTANT #5: Loading skeleton while products load */}
                {loading && (
                  <div className="products-grid">
                ...
                <div className="add-product-form">
                  ...
                  <form onSubmit={handleAddProduct}>
                    ...
                    <label className="image-upload-area" style={{ marginBottom: 12 }}>
                      <input type="file" accept="image/*" multiple onChange={handleImagesUpload} disabled={newProduct.images.length >= 8 || imageUploading} />
                      {imageUploading && <div style={{ fontSize: 11, color: '#24c9ff', marginTop: 4 }}>Uploading image(s)...</div>}
                    ...
                    <button type="submit" className="submit-btn" disabled={uploading || imageUploading}>{uploading ? 'Saving...' : 'Add Product'}</button>
                  </form>
                </div>
