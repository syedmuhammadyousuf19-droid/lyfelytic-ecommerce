...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .nav-btn {
    ...
    align-items: center; gap: 6px; color: white; font-size: 13px;
  ...
  .mobile-menu-btn { background: none; border: none; color: white; cursor: pointer; display: flex; align-items: center; }
  ...
  .category-chips { display: flex; gap: 8px; flex-wrap: wrap; flex: 1; }
  .chip {
    padding: 6px 14px; border-radius: 20px; font-size: 12px; font-weight: 600;
    cursor: pointer; transition: all 0.2s; border: 1px solid #c1b5a3;
    background: #ebe7df; color: #5a5a5a; white-space: nowrap;
  }
  .chip.active { background: #9d8b5f; border-color: #8a7850; color: white; }
  .chip:hover:not(.active) { background: #ddd6cc; color: #3a3a3a; }

  .filter-selects { display: flex; gap: 8px; }
  .filter-select {
    padding: 6px 12px; border-radius: 8px; font-size: 12px; font-weight: 600;
    background: #e8dfd5; border: 1px solid #c1b5a3;
    color: #3a3a3a; cursor: pointer; font-family: inherit; outline: none;
  }
  .filter-select option { background: var(--bg2); color: var(--text); }

  /* ── MAIN ── */
  .main-content { max-width: 1200px; margin: 20px auto; padding: 0 16px; margin-bottom: 40px; }
  .section-title { text-align: center; margin-bottom: 28px; }
  .section-title h2 { font-size: 26px; font-weight: 700; font-family: 'Playfair Display', serif; color: var(--text); margin-bottom: 6px; }
  .section-title p { color: var(--text-md); font-size: 14px; }
  ...
  .product-card:hover { transform: translateY(-4px); border-color: rgba(124,106,247,0.3); box-shadow: 0 12px 30px var(--shadow); }
  ...
  .wishlist-btn {
    ...
    background: rgba(15,15,26,0.7); border: none; border-radius: 50%;
    width: 34px; height: 34px; display: flex; align-items: center;
    justify-content: center; cursor: pointer; backdrop-filter: blur(6px);
    transition: all 0.2s; z-index: 5;
  }
  .wishlist-btn:hover { background: rgba(233,69,96,0.3); transform: scale(1.1); }
  .wishlist-btn.active { background: rgba(233,69,96,0.85); }
  ...
  .product-name { font-size: 15px; font-weight: 600; color: var(--text); margin-bottom: 5px; }
  .product-description { color: var(--text-md); font-size: 12px; margin-bottom: 10px; line-height: 1.5; }
  .product-price { font-size: 20px; font-weight: 700; color: var(--accent); margin-bottom: 4px; }
  ...
  .view-details-btn {
    ...
    background: rgba(124,106,247,0.12); color: #a89ff7;
    border: 1px solid rgba(124,106,247,0.3); border-radius: 8px;
    font-weight: 600; cursor: pointer; transition: all 0.25s;
    font-size: 13px; margin-bottom: 8px; backdrop-filter: blur(4px);
  }
  .view-details-btn:hover { background: rgba(124,106,247,0.25); }

  .add-to-cart-btn {
    width: 100%; padding: 10px;
    background: rgba(233,69,96,0.8); color: white;
    border: 1px solid rgba(233,69,96,0.5); border-radius: 8px;
    font-weight: 600; cursor: pointer; transition: all 0.25s;
    display: flex; align-items: center; justify-content: center;
    gap: 6px; font-size: 13px; backdrop-filter: blur(4px);
  }
  .add-to-cart-btn:hover:not(:disabled) { background: rgba(233,69,96,0.95); transform: translateY(-1px); }
  .add-to-cart-btn:disabled { background: rgba(255,255,255,0.06); color: var(--text-lt); border-color: var(--border); cursor: not-allowed; }
  ...
  .checkout-btn {
    ...
    background: rgba(233,69,96,0.85); color: white;
    border: 1px solid rgba(233,69,96,0.5); border-radius: 8px;
    font-weight: 700; cursor: pointer; transition: all 0.25s;
    display: flex; align-items: center; justify-content: center;
    gap: 6px; font-size: 13px; backdrop-filter: blur(4px);
  }
  .checkout-btn:hover { background: rgba(233,69,96,0.98); }
  ...
  .modal-title { font-size: 20px; font-weight: 700; font-family: 'Playfair Display', serif; color: var(--text); margin-bottom: 20px; display: flex; align-items: center; gap: 10px; }
  .modal-input {
  ...
  .modal-order-summary { background: var(--card); border-radius: 10px; padding: 14px; margin-bottom: 16px; }
  .modal-order-item { display: flex; justify-content: space-between; font-size: 13px; color: var(--text-md); padding: 4px 0; border-bottom: 1px solid var(--border); }
  .modal-order-item:last-child { border: none; }
  .modal-total { display: flex; justify-content: space-between; font-size: 15px; font-weight: 700; color: var(--text); margin-top: 10px; }
  .modal-btns { display: flex; gap: 10px; }
  .modal-cancel {
    flex: 1; padding: 12px; background: var(--glass); color: var(--text-md);
    border: 1px solid var(--border); border-radius: 8px; font-weight: 600;
    cursor: pointer; font-family: inherit; transition: background 0.2s;
  }
  .modal-cancel:hover { background: var(--glass-b); }
  .modal-confirm {
    flex: 2; padding: 12px; background: rgba(233,69,96,0.85); color: white;
    border: 1px solid rgba(233,69,96,0.5); border-radius: 8px; font-weight: 700;
    cursor: pointer; font-family: inherit; transition: background 0.2s;
    backdrop-filter: blur(4px);
  }
  .modal-confirm:hover { background: rgba(233,69,96,0.98); }
  ...
  .product-page-back {
    ...
    background: rgba(15,15,26,0.9); padding: 14px 16px;
    border: none; cursor: pointer; width: 100%;
    font-size: 15px; font-weight: 600; color: var(--accent2);
    border-bottom: 1px solid var(--border); transition: background 0.2s;
    backdrop-filter: blur(10px);
  }
  .product-page-back:hover { background: var(--bg2); }

  .gallery-wrapper { background: var(--bg2); position: relative; overflow: hidden; }
  .gallery-main { position: relative; width: 100%; height: 300px; overflow: hidden; touch-action: pan-y; }
  @media (min-width: 768px) { .gallery-main { height: 380px; } }
  .gallery-slides { display: flex; height: 100%; transition: transform 0.35s cubic-bezier(0.25,0.46,0.45,0.94); }
  .gallery-slide { min-width: 100%; height: 100%; display: flex; align-items: center; justify-content: center; background: var(--bg3); }
  .gallery-slide img { width: 100%; height: 100%; object-fit: contain; padding: 8px; }
  .gallery-arrow {
    position: absolute; top: 50%; transform: translateY(-50%);
    background: rgba(15,15,26,0.75); border: 1px solid var(--border);
    border-radius: 50%; width: 38px; height: 38px;
    display: flex; align-items: center; justify-content: center;
    cursor: pointer; backdrop-filter: blur(6px); z-index: 10; color: white; transition: background 0.2s;
  }
  .gallery-arrow:hover { background: rgba(124,106,247,0.4); }
  .gallery-arrow-left { left: 10px; }
  .gallery-arrow-right { right: 10px; }
  .gallery-arrow-hidden { opacity: 0; pointer-events: none; }
  .gallery-dots { display: flex; justify-content: center; gap: 6px; padding: 10px 0 6px; background: var(--bg2); }
  .gallery-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--text-lt); border: none; cursor: pointer; transition: all 0.25s; padding: 0; }
  .gallery-dot.active { background: var(--accent); width: 22px; border-radius: 4px; }
  .gallery-thumbs { display: flex; gap: 8px; padding: 10px 16px 14px; background: var(--bg2); overflow-x: auto; border-bottom: 1px solid var(--border); }
  .gallery-thumbs::-webkit-scrollbar { height: 3px; }
  .gallery-thumbs::-webkit-scrollbar-thumb { background: var(--text-lt); border-radius: 2px; }
  .gallery-thumb { flex-shrink: 0; width: 60px; height: 60px; border-radius: 8px; overflow: hidden; border: 2px solid transparent; cursor: pointer; transition: border-color 0.2s; background: var(--bg3); }
  ...
  /* ── FOOTER ── */
  .footer { background: rgba(15,15,26,0.95); color: var(--text-md); text-align: center; padding: 28px 20px; border-top: 1px solid var(--border); }
...
export default function LyfelyticEcommerce() {
  ...
  const [adminTab, setAdminTab] = useState('products'); // products | orders | dashboard
          ...
          <div className="nav-buttons">
            {!isAdmin && (
              <>
                ...
                <button onClick={() => setView('wishlist')} className="nav-btn" style={{ background: wishlist.length ? 'rgba(233,69,96,0.2)' : 'var(--glass)', border: '1px solid rgba(233,69,96,0.3)' }}>
                  <Heart size={15} fill={wishlist.length ? '#e94560' : 'none'} color="#e94560" /> {wishlist.length > 0 && wishlist.length}
                </button>
        ...
        <>
          ...
          <div className="filter-bar">
            <div className="category-chips">
              {CATEGORIES.map(cat => <button key={cat} className={`chip ${selectedCategory === cat ? 'active' : ''}`} onClick={() => setSelectedCategory(cat)}>{cat}</button>)}
            </div>
        ...
        {isAdmin && (
          <div className="admin-panel">
            ...
            <div className="admin-tabs">
              ...
              <button className={`admin-tab ${adminTab === 'dashboard' ? 'active' : ''}`} on
