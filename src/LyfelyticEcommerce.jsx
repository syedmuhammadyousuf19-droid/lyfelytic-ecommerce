...
const styles = `
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .dashboard-controls-info { display: flex; flex-direction: column; gap: 3px; }

  .dashboard-controls-title { color: var(--ink); font-size: 13px; font-weight: 800; }

  .dashboard-controls-sub { color: var(--slate-lt); font-size: 11px; }

  .dashboard-controls-actions { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }

  .reset-mode-select {
    padding: 9px 12px; border-radius: 8px; border: 1px solid var(--border);
    background: white; color: var(--ink); font-size: 12px; font-weight: 700;
    font-family: 'Manrope', sans-serif; outline: none; cursor: pointer;
  }

  .reset-mode-select:focus { border-color: var(--teal); box-shadow: 0 0 0 3px var(--teal-lt); }

  .reset-now-btn {
    padding: 9px 13px; border: 1px solid var(--coral-dark); border-radius: 8px;
    background: var(--coral); color: white; font-size: 12px; font-weight: 800;
    font-family: 'Manrope', sans-serif; cursor: pointer; transition: all 0.2s;
  }

  .reset-now-btn:hover:not(:disabled) { background: var(--coral-dark); transform: translateY(-1px); }

  .reset-now-btn:disabled { opacity: 0.6; cursor: wait; }

  @media (max-width: 600px) {
    .dashboard-controls { align-items: stretch; }
    .dashboard-controls-actions { width: 100%; }
    .reset-mode-select, .reset-now-btn { flex: 1; }
  }
...
export default function LyfelyticEcommerce() {
  ...
  // Dashboard reset controls: manual, daily, or monthly
  ...
  // Load dashboard reset settings from Supabase
  const fetchResetSettings = async () => {

    const { data, error } = await supabase
      .from('dashboard_settings')
      .select('reset_mode, reset_at')
      .eq('id', 1)
      .single();
  ...
  // Keep daily/monthly dashboard periods current while the admin panel is open.
  useEffect(() => {
    if (!isAdmin) return;
  ...
  // Manual reset changes reset_at. Daily/monthly modes use the current period boundary.
  const handleResetDashboard = async () => {

    if (!confirm('Reset dashboard statistics now? Your products, orders, reviews and categories will NOT be deleted.')) return;
    ...
    const { error } = await supabase
      .from('dashboard_settings')
      .update({ reset_at: now, updated_at: now })
      .eq('id', 1);

    if (error) {
      console.error(error);
      alert('Failed to reset dashboard. Please check the dashboard_settings table.');
    } else {
      setResetAt(now);
      setDashboardNow(Date.now());
      alert('Dashboard statistics have been reset successfully!');
    }
  ...
  const handleResetModeChange = async (mode) => {
    ...
    const { error } = await supabase
      .from('dashboard_settings')
      .update({ reset_mode: mode, reset_at: now, updated_at: now })
      .eq('id', 1);

    if (error) {
      console.error(error);
      setResetMode(resetMode);
      alert('Failed to change reset mode. Please check the dashboard_settings table.');
      return;
    }
  ...
  // Calculate the start of the dashboard's current reporting period.
  ...
  const periodOrders = orders.filter(o => {
    ...
    return new Date(o.created_at) >= dashboardPeriodStart;
  });
  ...
  const bestSelling = products
    .map(p => ({ ...p, orderCount: periodOrders.filter(o => o.items?.some(i => i.id === p.id)).length }))
            ...
            {adminTab === 'dashboard' && (
              ...
              <div>
                ...
                <div className="dashboard-controls">
                  ...
                  <div className="dashboard-controls-info">
                    <div className="dashboard-controls-title">📊 Dashboard Reset</div>
                    <div className="dashboard-controls-sub">{resetMode === 'daily' ? 'Stats reset at the start of each day.' : resetMode === 'monthly' ? 'Stats reset at the start of each month.' : `Stats since ${resetAt ? new Date(resetAt).toLocaleString() : 'the beginning'}.`}</div>
                  </div>

                  <div className="dashboard-controls-actions">
                    <select
                      className="reset-mode-select"
                      value={resetMode}
                      onChange={(e) => handleResetModeChange(e.target.value)}
                    >
                ...
                <div className="dashboard-grid">
                  ...
                  <div className="stat-card"><div className="stat-label">Total Orders</div><div className="stat-value">{periodOrders.length}</div><div className="stat-sub">Current reset period</div></div>

                  <div className="stat-card"><div className="stat-label">Revenue</div><div className="stat-value stat-accent">Rs.{totalRevenue.toLocaleString()}</div><div className="stat-sub">Delivered in period</div></div>

                  <div className="stat-card"><div className="stat-label">Pending</div><div className="stat-value stat-purple">{pendingOrders}</div><div className="stat-sub">In current period</div></div>
